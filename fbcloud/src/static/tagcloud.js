...
function TagCloud(cntr,sstyle,carray,chighlight,urlstub)
{
	...
	var url = urlstub || 'http://www.google.com/#sclient=psy&hl=en&aq=f&aqi=g4g-o1&aql=&oq=&gs_rfai=&pbx=1&fp=280187d6f0589da0&q='
	//var url = urlstub || 'http://search.twitter.com/search?q='
	...
	function draw()
	{
		...
		gridsize = Math.sqrt(maxsize / Math.PI) * 2 * 15;
	...
	function drawCircle(nd)
	{
		...
		var d = Math.sqrt(nd.size / Math.PI) * 2 * 15
		...
		dom.appendChild(cvs)
				...
				cvs.onclick = function(e) {
					...
					$("div.panel_button").show();
										...
										$('#friends_news_header').show();
                                        $('#everyone_news_header').show();
                                        $('#twitter_news_header').show();
