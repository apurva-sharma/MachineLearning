...
class HomeHandler(BaseHandler):
    def get(self):
        ...
        try:
            ...
            news_feed = self.graph.get_search_twitter(tag)
            #news_feed = self.graph.get_search_friends("hackathon")
        except:
            print "error1"
            news_feed = {"results": []}
        #for post in news_feed["results"]:
        #    post["created_at"] = datetime.datetime.strptime(
        #        post["created_at"], "%Y-%m-%dT%H:%M:%S+0000") + \
        #        datetime.timedelta(hours=7)

        self.render("twitter.html", news_feed=news_feed)
...
#            return
#
#        news_feed = urllib2.urlopen("http://search.twitter.com/search.json" + "?q=" + tag)
#	#output = file.read()
#	#print file
#	self.render("twitter.html", news_feed=news_feed)
#
...
def main():
    ...
    util.run_wsgi_app(webapp.WSGIApplication([
        (r"/searchtwitter", HomeHandler),
        (r"/searchtwitter", PostHandler),
    ], debug=debug))
